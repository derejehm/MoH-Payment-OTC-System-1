...
const HospitalPayment = () => {
  ...
  // Handle Amount Change for Selected Reasons
  const handleAmountChange = (event, reason) => {
    ...
    setFormData((prev) => ({
      ...
      reason: {
        ...prev.reason,
        [reason]: value, // Update amount for the selected reason
      },
    }));
  };
  ...
  const handleCheckboxChange = (e) => {
    ...
    setFormData((prev) => {
      const updatedReason = { ...prev.reason };

      if (!checked) {
        delete updatedReason[name]; // Remove key properly
      } else {
        updatedReason[name] = 0; // Add key with default value
      }

      return { ...prev, reason: updatedReason };
    });
  };
    ...
    <Container>
      ...
      <Paper sx={{ padding: 2, marginBottom: 2, display: "flex", gap: 2 }}>
        <Box sx={{ flex: 1 }}>
          ...
          {reasons.map((reason) => (
            <FormControlLabel
              ...
              control={
                <Checkbox
                  name={reason}
                  checked={formData.reason.hasOwnProperty(reason)}
          ...
          {Object.keys(formData.reason).map((name) => (
            <TextField
              key={name}
              label={`${name} Amount`}
              fullWidth
              margin="normal"
              value={formData.reason[name]}
